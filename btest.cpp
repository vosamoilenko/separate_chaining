...
void test_insert(ads::set<val_t>& a, std::set<val_t>& r, size_t n, size_t max_value, RNG&& gen) {
    ...
    for(size_t i = 0; i < n; ++i) {
        ...
        // std::cerr << RED(*it_a.first << "\n");
        // std::cerr << RED(*it_r.first << "\n");
...
void stresstest() {
    ...
    if(f.wait_for(std::chrono::seconds(100)) == std::future_status::timeout) {
        std::cerr << YELLOW("[stresstest12] timeout: exceeded 1s timeframe.\n");
        std::abort();
    }
    ...
    if(g.wait_for(std::chrono::seconds(200)) == std::future_status::timeout) {
        std::cerr << YELLOW("[stresstest22] timeout: exceeded 2s timeframe.\n");
        std::abort();
    }
#endif
}
